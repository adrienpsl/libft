	...
	* */
	{
		...
		// test no array
		{
			ret = ft_array$bubble(NULL, ft_array$sort_cmp_int, &param);
			if (!ret
				|| lib_cmp_testbuff(
		...
		// test no param
		{
			...
			ft_array$push(&array, &param);
			ret = ft_array$bubble(array, ft_array$sort_cmp_int, NULL);
			if (ret
				|| lib_cmp_testbuff(
	...
	* */
	{
		...
		// test with 0 number
		{
			ret = ft_array$bubble(array, ft_array$sort_cmp_int, &order);
			if (ret)
				log_test(3)
		}
