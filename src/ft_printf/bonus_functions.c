...
void			color(t_printf *p)
{
	...
	p->printed = 5;
	if (ft_strneq(p->format, "{red}", (len = 5)))
		buffer(p, PF_RED, p->printed);
	else if (ft_strneq(p->format, "{green}", (len = 7)))
		buffer(p, PF_GREEN, p->printed);
	else if (ft_strneq(p->format, "{yellow}", (len = 8)))
		buffer(p, PF_YELLOW, p->printed);
	else if (ft_strneq(p->format, "{blue}", (len = 6)))
		buffer(p, PF_BLUE, p->printed);
	else if (ft_strneq(p->format, "{purple}", (len = 8)))
		buffer(p, PF_PURPLE, p->printed);
	else if (ft_strneq(p->format, "{cyan}", (len = 6)))
		buffer(p, PF_CYAN, p->printed);
	else if (ft_strneq(p->format, "{eoc}", (len = 5)))
